...
# July 2021
# This script collects the f0 listing of the reference tier (ref_tier) of your selected speaker.
...
# creating a list of files for the specified speaker
speakers = Create Strings from tokens: "speakers", chosen_speaker$, " ,"
numberOfSpeakers = Get number of strings
for ispeaker to numberOfSpeakers
	select Strings speakers
	speaker$ = Get string: ispeaker
	strings = Create Strings as file list: string$(ispeaker), sound_directory$ + "*"+ speaker$ + "*.wav"
endfor

list_num = 1
selectObject: "Strings " + string$(list_num)
if numberOfSpeakers != 1
	repeat
	# select Strings 'string$(list_num)'
	# plus Strings 'string$(list_num + 1)'

	plusObject: "Strings " + string$(list_num + 1)
		list_num = list_num + 1
	until list_num = numberOfSpeakers

	Append
endif
Rename: "list"
pause Check the list -- Are these the right files? 
